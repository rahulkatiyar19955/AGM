...
rule:
  STRING ':' STRING ENDLS '{' ENDLS graphL GETS ENDLS graphR '}' ENDLS {rule.computeEffects(); rule.setName($1); rule.setActive($3);agmRules->push_back(rule); rule.clear(); printf("R %d\n", agmRules->size());}
;
