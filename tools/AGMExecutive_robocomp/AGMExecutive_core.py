...
class Executive(object):
    ...
    def edgesUpdate(self, edges):
        try:
            ...
            for edge in edges:
                ...
                found = False
                for i in range(len(self.currentModel.links)):
    ...
    def addSelfEdge(self, identifier, edge_type, attributes):
        try:
            self.mutex.acquire()
            found = False
            for i in range(len(self.currentModel.links)):
                if str(self.currentModel.links[i].a) == str(identifier):
                    if str(self.currentModel.links[i].b) == str(identifier):
                        if str(self.currentModel.links[i].linkType) == str(edge_type):
                            self.currentModel.links[i].attributes = copy.deepcopy(attributes)
                            found = True
                            break
            if not found:
                ...
                if str(self.worldModelICE.edges[i].a) == str(identifier):
                    if str(self.worldModelICE.edges[i].b) == str(identifier):
                        if str(self.worldModelICE.edges[i].edgeType) == str(edge_type):
                            ...
                            break
            if not found:
                new_edge = RoboCompAGMWorldModel.Edge()
                new_edge.a = identifier
                new_edge.b = identifier
                new_edge.attributes = attributes
                self.worldModelICE.edges.append(new_edge)
        finally:
            self.mutex.release()
    ...
    def gotPlan(self, plan):
        ...
        for p in list(parameterMap.keys()):
