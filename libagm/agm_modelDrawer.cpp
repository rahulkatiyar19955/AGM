...
AGMModelDrawer::AGMModelDrawer(RCDraw *drawer_, QTableWidget *tableWidget_)
{
    drawer = drawer_;
...
AGMModelDrawer::~AGMModelDrawer()
{
    
}

void AGMModelDrawer::setInterest(std::string item)
{
    interest = item;
    drawTable();
}
...
void AGMModelDrawer::update(const AGMModel::SPtr &w)
{
...
void AGMModelDrawer::setShowMesh(bool s)
{
    showMesh=s;
    meshList.clear();
...
void AGMModelDrawer::setShowPlane (bool s)
{
    showPlane=s;
    planeList.clear();
    for (uint32_t i=0; i<model->symbols.size(); ++i)
...
void AGMModelDrawer::setShowInnerModel(bool s)
{
    showInner = s;
}

void AGMModelDrawer::setShowRobot( bool s)
{
    if (s==true)
    {
        showRobot = s;
...
void AGMModelDrawer::listOfSymbolThroughLinkType( int& symbolID, QList<int> &visited, std::string linkType, bool &loop)
{
...
void AGMModelDrawer::recalculatePositions()
{
...
void AGMModelDrawer::updateStructure()
{
    // Push back new nodes
...
//             if (nodes[n].edgesOriented[e] == n) printf("self-edge: %s\n", nodes[n].edgesNames[e].c_str());
            ...
            QPointF inc = (p2 - p1);
            float divv = sqrt(inc.x()*inc.x() + inc.y()*inc.y());
            if (fabs(divv)>0.0001)
            {
                inc /= divv;
            }
            ...
            if (nodes[n].edgesOriented[e] == n)
            {
                labelTextPosition += QPointF(0, radius+15);
            }
...
void AGMModelDrawer::clickedNode(QPointF pC)
{
