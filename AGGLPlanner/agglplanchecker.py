...
class PyPlanChecker(object):
	...
	def __init__(self, agmData, domainPath, init, planPath, targetPath, resultPath='', verbose=True):
			...
			for action in self.plan:
				...
				for p in action.parameters.keys():
					if not action.parameters[p] in world.graph.nodes.keys():
						par = action.parameters[p]
						for r in agmData.agm.rules:
							if r.name == action.name:
								rhs = r.rhs
						print p, par, rhs.nodes.keys()
						if not (p in rhs.nodes.keys()):
							# If the parameter doesnt exit in the world, we raise an exception.
